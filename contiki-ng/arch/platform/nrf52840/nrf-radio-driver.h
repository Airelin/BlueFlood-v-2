...
#define PRINT_RX_STATS_CONF true //false
...
#endif
/*---------------------------------------------------------------------------*/
#define SLOT_PROCESSING_TIME_PKT_END    (US_TO_RTIMERTICKS(8)) //sometimes changed to 16
...
/*---------------------------------------------------------------------------*/
/* Do not tx/rx data, but transmit a constant unmodulated carrier 
 * (by keeping the radio in idle mode in slots instead of starting tx/rx) 
 */
...
#if (RADIO_MODE_CONF == RADIO_MODE_MODE_Ieee802154_250Kbit)
  #define get_rx_ts() (NRF_TIMER0->CC[1] - 1) //correct for custom PPI channel latency
  #define get_rx_rssi(MSG) ((MSG==0)? get_radio_rssi() : ((ble_beacon_t *)(MSG))->lqi)
#else
  #define get_rx_ts() (NRF_TIMER0->CC[1])
