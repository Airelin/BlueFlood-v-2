...
void schedule_tx(uint8_t* buf, int channel, rtimer_clock_t t)
{
  ...
  rtimer_clock_t t_abs = t + NRF_TIMER0->CC[3];
...
{  
  ...
  NRF_PPI->CH[RTC_FIRE_PPI_CH].EEP = (uint32_t)&NRF_RTC2->EVENTS_COMPARE[1];
  ...
  // NRF_PPI->CH[RTC_SCHEDULE_PPI_CH].EEP = (uint32_t)&NRF_RTC2->EVENTS_OVRFLW;
