...
PROCESS_THREAD(tx_process, ev, data)
{
  ...
  //For evaluating round_fixed version:
  ...
  while(1)
  {
    ...
    for(slot = 0; ROUND_LEN_RULE; slot++){
      ...
      if(do_tx){
        ...
        #if PRINT_FIXED_ROUND
        schedule_tx_abs(tx_msg, GET_CHANNEL(round_fix,slot), tt - ADDRESS_EVENT_T_TX_OFFSET + ARTIFICIAL_TX_OFFSET);
        #else
