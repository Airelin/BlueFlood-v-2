...
PROCESS_THREAD(tx_process, ev, data)
{
  ...
  //Some paket ingredience and log stuff
  ...
  // interval to start earlier to listen
  uint32_t guard_time = 0;
  int i;
  uint8_t last_rx_ok = 0;
  PROCESS_BEGIN();

  //Evaluation testbed to get node id's
  #if TEST_HELLO_WORLD
  ...
  //Send node to sleep if you have a testbed with more nodes than you want
  ...
  while(1)
  {
    ...
    for(slot = 0; ROUND_LEN_RULE; slot++){
      ...
      if(do_tx){
        ...
        //For evaluation of BlueFlood (CE = Cuncurrent Transmission Evaluation)
        #if TEST_CE
        ...
        do{
          ...
          if(got_address_event) {
            ...
            #else
            //get payload event before getting end event
